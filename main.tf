...
resource "azurerm_virtual_network" "vnet" {
  ...
  address_space       = ["10.0.0.0/16"]      # ✅ ye hona zaroori hai
...
resource "azurerm_subnet" "subnet1" {
  ...
  address_prefixes     = ["10.0.10.0/24"] // Changed range
}
...
resource "azurerm_subnet" "subnet2" {
  ...
  resource_group_name  = azurerm_resource_group.rg.name
  virtual_network_name = azurerm_virtual_network.vnet.name
  address_prefixes     = ["10.0.20.0/24"] // Changed range
}
...
data "azurerm_public_ip" "assigned" {
...
output "vm_public_ip" {
  value       = data.azurerm_public_ip.assigned.ip_address
  description = "Public IP of the VM after allocation"
}
